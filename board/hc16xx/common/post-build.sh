...
if [ -f ${DTB} ] ; then
	...
	sed -i "s/part${fw_part}-filename =.*/part${fw_part}-filename = \"${CONFIG_APPS_NAME}.uImage\";/g" ${DTB}.dts
...
if [ "${BR2_EXTERNAL_BOOT_TYPE_SPINAND}" = "y" ]; then
boot_type=0x00000001
else
boot_type=0x00000000
fi

spinandwr_ep_noncache=$(readelf -h ${TOPDIR}/build/tools/spinandwr.out | grep Entry | awk '{print $NF}' | sed 's/0x8/0xa/')
spinandwr_load_noncache=$(nm -n ${TOPDIR}/build/tools/spinandwr.out | awk '/T _start/ {print "0x"$1}' | sed 's/0x8/0xa/')

cat << EOF > ${IMAGES_DIR}/sfburn.ini
[Project]
RunMode=0
InitMode=0
RunAddr=${spinandwr_ep_noncache}
FileNum=2
[File0]
...
File=spinandwr.out
Type=5
Addr=${spinandwr_load_noncache}
[AutoRun]
AutoRun0=wm 0x800001F0 $(printf 0x%08x $sfbin_size_align)
AutoRun1=wm 0x800001F4 0x00000000
AutoRun2=wm 0x800001F8 0x81000000
AutoRun3=wm 0x800001FC ${boot_type}
AutoRun4=wm 0xb8818504 0x0
EOF
