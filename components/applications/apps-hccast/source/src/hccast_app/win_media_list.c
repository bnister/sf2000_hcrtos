...
static uint16_t win_get_file_idx_name(file_list_t *file_list, char *file_name);
...
static bool win_media_is_root_dir(file_list_t *file_list)
{
    if (strcmp(file_list->dir_path, ROOT_DIR))
        return false;
    else
        return true;
}

static void win_get_parent_dirname(char *parentpath, char *path)
...
static uint16_t win_get_file_idx_name(file_list_t *file_list, char *file_name)
{
    ...
    while (list_tmp){
        ...
        if (!strcmp(file_name, file_node->name)){
            found = true;
            break;
        }
...
static uint16_t win_get_file_idx_fullname(file_list_t *file_list, char *file_name)
{
    glist *list_tmp;
    file_node_t *file_node;
    uint16_t index = 0;
...
printf("%s(), line:%d, full_name:%s, file_name:%s \n", __FUNCTION__, __LINE__, full_name, file_name);                
...
static int win_media_list_open(void *arg)
{
    ...
    lv_obj_set_scrollbar_mode(m_frame_path, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_size(m_frame_path, MEDIA_PATH_W, MEDIA_PATH_H);
    lv_obj_align(m_frame_path, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(m_frame_path, COLOR_DEEP_GREY, LV_PART_MAIN);
    ...
    if ('\0' == m_cur_file_list->dir_path[0])
        strncpy(m_cur_file_list->dir_path, ROOT_DIR, MAX_FILE_NAME-1);
...
static uint16_t win_list_ctl_proc(obj_list_ctrl_t *list_ctrl, uint16_t vkey, uint16_t pos)
{
    ...
    focus_idx = item_new_pos - list_ctrl->start;
printf("%s(),line:%d. focus_idx:%d, item_new_pos:%d\n", __FUNCTION__, __LINE__, 
    focus_idx, item_new_pos);    
    lv_group_focus_obj(m_list_label[focus_idx]);
