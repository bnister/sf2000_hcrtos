...
static int tv_sys_match_tv_type_get(int tv_type){
	...
	if (tv_type >= TV_LINE_3840X2160_30){
		return TV_LINE_3840X2160_30;
	} else if (tv_type >= TV_LINE_1080_60){
		return TV_LINE_1080_60;
	} else if (tv_type >= TV_LINE_720_30){
		return TV_LINE_720_30;
	} else if (tv_type >= TV_NTSC){
		return TV_NTSC;
	} else if (tv_type >= TV_PAL){
		return TV_PAL;
	} else {
		return TV_PAL;
	}
...
static int tv_sys_auto_set(bool dual_out, int set_tv_type, bool auto_set, uint32_t timeout)
{
	int fd_dis = -1;
	struct dis_tvsys tvsys = { 0 };	
	int dac_tpye = DIS_DAC_CVBS;
