...
static int win_vol_update(uint32_t param)
{
    ...
    if (m_set_vol){
		vol_get = projector_get_some_sys_param(P_VOLUME);
	    if (V_KEY_V_UP == key){
	    	if (vol_get >= 90)
	    		volume = 100;
	    	else
    	...
    	api_media_set_vol(volume);
    	projector_set_some_sys_param(P_VOLUME, volume);
    }
...
static void win_volume_control(void *arg1, void *arg2)
{
