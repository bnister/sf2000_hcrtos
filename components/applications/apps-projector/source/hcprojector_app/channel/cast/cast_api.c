...
void cast_usb_mirror_rotate_init(void)
{
    ...
    um_param.full_screen_en = projector_get_some_sys_param(P_SYS_ZOOM_OUT_COUNT) == 0 ? 1 : 0;
...
int cast_init(void)
{
	...
	hccast_mira_res_e res = HCCAST_MIRA_RES_720P30;
...
int hccast_mira_callback_func(hccast_mira_event_e event, void* in, void* out)
{
	...
	if((event != HCCAST_MIRA_GET_MIRROR_ROTATION) && (event != HCCAST_MIRA_GET_MIRROR_VSCREEN_AUTO_ROTATION) \
                    && (event != HCCAST_MIRA_GET_FLIP_MODE) )
	{
		log(DEMO, INFO, "[%s] event: %d", __func__, event);
	}	
    ...
    {
        //*(int*)in = projector_get_some_sys_param(P_SYS_ZOOM_OUT_COUNT) == 0 ? 1 : 0;
        break;
    }
...
int hccast_air_callback_event(hccast_air_event_e event, void* in, void* out)
{
    ...
    switch (event)
    {
        ...
        case HCCAST_AIR_GET_MIRROR_MODE:
            ...
            *(int*)in = projector_get_some_sys_param(P_AIRCAST_MODE);
            break;
