...
#define kMatchSpecLenStart                                                     \
	(kMatchMinLen + kLenNumLowSymbols + kLenNumMidSymbols +                \
	 kLenNumHighSymbols)
	...
	LzmaDec_DecodeReal(CLzmaDec *p, SizeT limit, const Byte *bufLimit)
{
	...
	UInt32 rep0 = p->reps[0], rep1 = p->reps[1], rep2 = p->reps[2],
	       rep3 = p->reps[3];
...

		{
			...
			if (state >= kNumStates) {
				...
				if (distance >= kStartPosModelIndex) {
					...
					unsigned numDirectBits = (unsigned)((
						(distance >> 1) - 1));
              ...
              */
				...
				state = (state < kNumStates + kNumLitStates) ?
						kNumLitStates :
						kNumLitStates + 3;
			}
...
static void MY_FAST_CALL LzmaDec_WriteRem(CLzmaDec *p, SizeT limit)
{
	if (p->remainLen != 0 && p->remainLen < kMatchSpecLenStart) {
		...
		SizeT rep0 =
			p->reps[0]; /* we use SizeT to avoid the BUG of VC14 for AMD64 */
		SizeT rem = limit - dicPos;
		if (rem < len)
			len = (unsigned)(rem);

		if (p->checkDicSize == 0 &&
		    p->prop.dicSize - p->processedPos <= len)
...
static int MY_FAST_CALL LzmaDec_DecodeReal2(CLzmaDec *p, SizeT limit,
					    const Byte *bufLimit)
{
	do {
		SizeT limit2 = limit;
	...
	} while (p->dicPos < limit && p->buf < bufLimit &&
...
static ELzmaDummy LzmaDec_TryDummy(const CLzmaDec *p, const Byte *buf,
				   SizeT inSize)
{
	UInt32 range = p->range;
	UInt32 code = p->code;
			...
			if (state < 4) {
				...
				prob = probs + PosSlot +
				       ((len < kNumLenToPosStates ?
						 ...
						 kNumLenToPosStates - 1)
					<< kNumPosSlotBits);
				TREE_DECODE_CHECK(prob, 1 << kNumPosSlotBits,
						  posSlot);
...
SRes LzmaDec_DecodeToDic(CLzmaDec *p, SizeT dicLimit, const Byte *src,
	...
	while (p->remainLen != kMatchSpecLenStart) {
		...
		if (p->tempBufSize == 0) {
			...
			if (rem < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow) {
				int dummyRes =
					LzmaDec_TryDummy(p, p->tempBuf, rem);
				if (dummyRes == DUMMY_ERROR) {
					(*srcLen) += lookAhead;
					*status = LZMA_STATUS_NEEDS_MORE_INPUT;
					return SZ_OK;
			 ...
			 ELzmaFinishMode finishMode, ELzmaStatus *status)
{
	...
	*srcLen = *destLen = 0;
	for (;;) {
		...
		res = LzmaDec_DecodeToDic(p, outSizeCur, src, &inSizeCur,
...
SRes LzmaProps_Decode(CLzmaProps *p, const Byte *data, unsigned size)
{
	...
	if (size < LZMA_PROPS_SIZE)
		...
		dicSize = data[1] | ((UInt32)data[2] << 8) |
				   ...
				   ISzAlloc *alloc)
{
	...
	if (!p->probs || numProbs != p->numProbs) {
		LzmaDec_FreeProbs(p, alloc);
		p->probs = (CLzmaProb *)alloc->Alloc(
			alloc, numProbs * sizeof(CLzmaProb));
		p->numProbs = numProbs;
		if (!p->probs)
			return SZ_ERROR_MEM;
	}
...
SRes LzmaDec_AllocateProbs(CLzmaDec *p, const Byte *props, unsigned propsSize,
			   ISzAlloc *alloc)
{
	CLzmaProps propNew;
...
SRes LzmaDecompress(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
