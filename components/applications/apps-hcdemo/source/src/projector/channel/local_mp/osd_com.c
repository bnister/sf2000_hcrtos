...
void win_msgbox_msg_open(const char *str_msg, uint32_t timeout, msg_timeout_func timeout_func, void *user_data)
{
    ...
    // m_obj_msg = lv_obj_create(ui_ctrl_bar);
    ...
    // lv_label_set_text(txt_msg, str_msg);    //to do here 
/////////////////////////////////////////////////////    
    if (NULL == m_obj_msg){
        m_obj_msg = lv_obj_create(ui_ctrl_bar);

        lv_obj_set_width(m_obj_msg, 450);
        lv_obj_set_height(m_obj_msg, 250);

        lv_obj_set_x(m_obj_msg, 0);
        lv_obj_set_y(m_obj_msg, -100);
    ...
    lv_obj_set_align(m_obj_msg, LV_ALIGN_CENTER);
        ...
        lv_obj_set_style_bg_opa(m_obj_msg, 100, LV_PART_MAIN | LV_STATE_DEFAULT);
