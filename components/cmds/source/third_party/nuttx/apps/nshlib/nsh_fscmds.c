...
#if (defined(CONFIG_FSUTILS_MKFATFS) || defined(CONFIG_FSUTILS_FF_MKFATFS))
  ...
  if ((lsflags & (LSFLAGS_SIZE | LSFLAGS_LONG)) != 0)
    {
      ...
      if ((lsflags & LSFLAGS_SIZE) != 0)
        {
          nsh_output(vtbl, " %10d", buf.st_size);
        }
    }
...
int _cmd_cat(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_cat(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(cat, "nsh", _cmd_cat, CONSOLE_CMD_MODE_SELF, "<path> [<path> [<path> ...]]")
...
int _cmd_cp(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_cp(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(cp, "nsh", _cmd_cp, CONSOLE_CMD_MODE_SELF, "<source-path> <dest-path>")
...
int _cmd_ls(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_ls(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(ls, "nsh", _cmd_ls, CONSOLE_CMD_MODE_SELF, "[-lRs] <dir-path>")
...
int _cmd_mkdir(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_cat(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(mkdir, "nsh", _cmd_mkdir, CONSOLE_CMD_MODE_SELF, "<path>")
 ...
 * Name: cmd_mkfatfs
 ****************************************************************************/
#if !defined(CONFIG_DISABLE_MOUNTPOINT) && defined(CONFIG_FS_ROMFS)
#ifndef CONFIG_NSH_DISABLE_ROMFSTEST
#ifndef CONFIG_NSH_ROMFS_IMAGEFILE
#  error "CONFIG_NSH_ROMFS_IMAGEFILE must be defined"
#endif

#ifndef CONFIG_NSH_ROMFS_DEV_MINOR
#  error "CONFIG_NSH_ROMFS_DEV_MINOR must be defined"
#endif

#ifndef CONFIG_NSH_ROMFS_MOUNTPOINT
#  error "CONFIG_NSH_ROMFS_MOUNTPOINT must be defined"
#endif

#define STR2(m)  #m
#define STR(m) STR2(m)
#define MKMOUNT_DEVNAME(m) "/dev/ram" STR(m)
#define MOUNT_DEVNAME      MKMOUNT_DEVNAME(CONFIG_NSH_ROMFS_DEV_MINOR)
...
"romfs_data_begin:\n"
	".incbin " STR(CONFIG_NSH_ROMFS_IMAGEFILE) "\n"\
	\
	".balign 64\n"
...
int _cmd_romfstest(int argc, char **argv)
{
	uint32_t romfs_data_len;
	int ret;

	romfs_data_len = (uintptr_t)&romfs_data_end - (uintptr_t)&romfs_data_begin;
	ret = romdisk_register(1, &romfs_data_begin,
	                       NSECTORS(romfs_data_len), ROMFS_SECTOR_SIZE);
	if (ret < 0) {
		printf("ERROR: romdisk_register failed: %d\n", -ret);
		return ret;
	}
	ret = mount(MOUNT_DEVNAME, CONFIG_NSH_ROMFS_MOUNTPOINT, "romfs", MS_RDONLY, NULL);
	if (ret < 0) {
		printf("ERROR: mount(%s,%s,romfs) failed: %d\n", MOUNT_DEVNAME,
		     CONFIG_NSH_ROMFS_MOUNTPOINT, ret);
		return ret;
	}

	return OK;
}
NSH_CONSOLE_CMD(romfstest, "nsh", _cmd_romfstest, CONSOLE_CMD_MODE_SELF, NULL)
#endif
#endif

#if !defined(CONFIG_DISABLE_MOUNTPOINT) && (defined(CONFIG_FSUTILS_MKFATFS) || defined(CONFIG_FSUTILS_FF_MKFATFS))
#ifndef CONFIG_NSH_DISABLE_MKFATFS
int _cmd_mkfatfs(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_mkfatfs(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(mkfatfs, "nsh", _cmd_mkfatfs, CONSOLE_CMD_MODE_SELF, "[-F <fatsize>] [-r <rootdirentries>] <block-driver>")
      ...
      switch (option)
        {
         ...
         case 'r':
            ...
            if ((short)(fmt.ff_rootdirentries) < 0)
              {
                nsh_error(vtbl, g_fmtargrange, argv[0]);
                badarg = true;
              }
 ...
 * Name: cmd_mkrd
 ****************************************************************************/
#if !defined(CONFIG_NSH_DISABLE_MKRD) && defined(CONFIG_DRVR_MKRD)
int _cmd_mkrd(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_mkrd(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(mkrd, "nsh", _cmd_mkrd, CONSOLE_CMD_MODE_SELF, "[-m <minor>] [-s <sector-size>] <nsectors>")
int cmd_mkrd(FAR struct nsh_vtbl_s *vtbl, int argc, char **argv)
{
  ...
  ret = mkrd(minor, nsectors, sectsize,RDFLAG_WRENABLED | RDFLAG_FUNLINK);
  if (ret < 0)
    {
...
int _cmd_mv(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_mv(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(mv, "nsh", _cmd_mv, CONSOLE_CMD_MODE_SELF, "<old-path> <new-path>")
...
int _cmd_rm(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_rm(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(rm, "nsh", _cmd_rm, CONSOLE_CMD_MODE_SELF, "<file-path>")
...
int _cmd_rmdir(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	cmd_rmdir(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(rmdir, "nsh", _cmd_rmdir, CONSOLE_CMD_MODE_SELF, "<dir-path>")
