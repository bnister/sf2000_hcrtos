...
int _cmd_mount(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	return cmd_mount(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(mount, "nsh", _cmd_mount, CONSOLE_CMD_MODE_SELF, "[-t <fstype> [-o <options>] [<block-device>] <mount-point>]")
...
int _cmd_umount(int argc, char **argv)
{
	opterr = 0;
	optind = 0;
	return cmd_umount(&g_vtbl, argc, argv);
}
NSH_CONSOLE_CMD(umount, "nsh", _cmd_umount, CONSOLE_CMD_MODE_SELF, "<dir-path>")
