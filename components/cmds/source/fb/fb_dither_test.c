...
static int fb_dither_start (int argc , char *argv[]) {
	...
	printf("bits_per_pixel: %d,red.length: %d, green.length: %d, blue.length: %d\n",
	       (unsigned int)vinfo.bits_per_pixel, (unsigned int)vinfo.red.length, (unsigned int)vinfo.green.length,
	...
	printf("pixel_bytes: %d, screen_size: %08x,fb_p0 = 0x%x,fb_dis_buf=0x%x\n", (unsigned int)pixel_bytes, \
		(unsigned int)screen_size,(unsigned int)fb_p0,(unsigned int)fb_dis_buf);
...
CONSOLE_CMD(dither_start, "fb", fb_dither_start, CONSOLE_CMD_MODE_SELF, "fb dither demo open")
CONSOLE_CMD(dither_stop, "fb", fb_dither_stop, CONSOLE_CMD_MODE_SELF,"fb dither demo close")
