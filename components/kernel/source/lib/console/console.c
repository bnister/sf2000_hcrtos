...
{
	int ret = CONSOLE_OK; /* Set to CONSOLE_QUIT if want to quit console */
...
#define QUOT 0x80
static void remove_chunk(char *char_buf, int beg, int end)
{
	/* beg must be <= end */
	if (beg == end)
		return;

	while ((char_buf[beg] = char_buf[end]) != 0)
		beg++, end++;
}

static void revert_quot(char *char_buf, int len)
{
	int i;
	for (i = 0; i < len; i++) {
		if (char_buf[i] == '\\') {
			remove_chunk(char_buf, i, i + 1);
			char_buf[i] &= (~QUOT);
		}
	}
}

static void mark_quot(const char *line)
{
	char *char_buf = (char *)line;
	int i;

	for (i = 0; char_buf[i]; i++) {
		if (char_buf[i] == '\\' && char_buf[i + 1])
			char_buf[i + 1] |= QUOT;
	}
}
...
static void console_register_init_cmds(const char *cmd)
{
	struct console_init_cmd *cmd_start =
		(struct console_init_cmd *)&_console_init_start;
	struct console_init_cmd *cmd_end =
		(struct console_init_cmd *)&_console_init_end;
	...
	for (p = cmd_start; p < cmd_end; p++) {
		...
		if (p->parent) {
			console_register_init_cmds(p->parent);
			parent = console_find_cmd(p->parent);
			console_register_cmd(parent, p->cmd, p->callback, p->mode,
				     p->help);
			p->initialized = true;
			return;
		} else {
			console_register_cmd(NULL, p->cmd, p->callback,
					p->mode, p->help);
			p->initialized = true;
			return;
		}
...
void console_init(void)
{
	...
	struct console_init_cmd *cmd_end =
		(struct console_init_cmd *)&_console_init_end;
	struct console_init_cmd *p;
	int np = fdt_get_node_offset_by_path("/hcrtos/board");
	const char *path = NULL;

	fdt_get_property_string_index(np, "label", 0, &path);
	...
	if (path)
		console_set_hostname(path);
	else
		console_set_hostname("root@local:");
	...
	for (p = cmd_start; p < cmd_end; p++) {
		if (p->initialized)
			continue;
		console_register_init_cmds(p->cmd);
	}
...
void console_start(void)
{
	...
	fp = fopen("/etc/rcS", "r");
	if (fp != NULL) {
		while ((read = compat_getline(&line, &len, fp)) != -1) {
			console_run_cmd(line);
		}
