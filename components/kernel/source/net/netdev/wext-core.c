...
/* IW event code */
#define WPA_NETLINK_WPA_IFACE_PORT_IN 9880
#define WPA_NETLINK_WPA_IFACE_PORT_OUT 9881

#define WPA_NETLINK_P2P_IFACE_PORT_IN 9882
#define WPA_NETLINK_P2P_IFACE_PORT_OUT 9883

#define WPA_NETLINK_HOSTAPD_IFACE_PORT_IN 9884
#define WPA_NETLINK_HOSTAPD_IFACE_PORT_OUT 9885

static int nlsock_wpa = -1;
static int nlsock_p2p = -1;
static int nlsock_hostapd = -1;
static struct sockaddr_in nlsaddr_wpa, nldaddr_wpa;
static struct sockaddr_in nlsaddr_p2p, nldaddr_p2p;
static struct sockaddr_in nlsaddr_hostapd, nldaddr_hostapd;

static int create_sock(struct sockaddr_in *src, struct sockaddr_in *dst, int port_in, int port_out)
{
	int sock;
	...
	src->sin_addr.s_addr = htonl((127 << 24) | 1);
	src->sin_port = htons(port_in);

	memset(dst, 0, sizeof(*dst));
	dst->sin_family = AF_INET;
	dst->sin_addr.s_addr = htonl((127 << 24) | 1);
	dst->sin_port = htons(port_out);
...
static int nlsock_init(void)
{
	if(nlsock_wpa < 0)
		nlsock_wpa = create_sock(&nlsaddr_wpa, &nldaddr_wpa, WPA_NETLINK_WPA_IFACE_PORT_IN, WPA_NETLINK_WPA_IFACE_PORT_OUT);
	if(nlsock_p2p < 0)
		nlsock_p2p = create_sock(&nlsaddr_p2p, &nldaddr_p2p, WPA_NETLINK_P2P_IFACE_PORT_IN, WPA_NETLINK_P2P_IFACE_PORT_OUT);
	if(nlsock_hostapd < 0)
		nlsock_hostapd = create_sock(&nlsaddr_hostapd, &nldaddr_hostapd, WPA_NETLINK_HOSTAPD_IFACE_PORT_IN, WPA_NETLINK_HOSTAPD_IFACE_PORT_OUT);
	return 0;
}

static void nlmsg_send(struct sk_buff *skb)
{
	if (nlsock_wpa < 0 || nlsock_p2p < 0 || nlsock_hostapd < 0) {
		nlsock_init();
		if (nlsock_wpa < 0 || nlsock_p2p < 0 || nlsock_hostapd < 0) {
			netdev_err(dev, "create netlink sock failed, drop nlmsg\n");
			return;
		}
	}

	sendto(nlsock_wpa, skb->data, skb->len, 0, (struct sockaddr *) &nldaddr_wpa, sizeof(nldaddr_wpa));
	sendto(nlsock_p2p, skb->data, skb->len, 0, (struct sockaddr *) &nldaddr_p2p, sizeof(nldaddr_p2p));
	sendto(nlsock_hostapd, skb->data, skb->len, 0, (struct sockaddr *) &nldaddr_hostapd, sizeof(nldaddr_hostapd));
}

extern unsigned lwip_netif_get_flags(struct netif *netif);
					      ...
					      struct sk_buff *skb)
{
	...
	r->__ifi_pad = 0;
	r->ifi_type = dev->type;
	r->ifi_index = netif_get_index(dev->netif);
	r->ifi_flags = lwip_netif_get_flags(dev->netif);
 ...
 */
static int wireless_process_ioctl(struct net_device *dev, struct ifreq *ifr,
				  unsigned int cmd,
				  struct iw_request_info *info,
...
static int wext_ioctl_dispatch(struct net_device *net, struct ifreq *ifr,
...
int wext_handle_ioctl(struct netif *netif, struct ifreq *ifr, unsigned int cmd, void *arg)
{
	struct iw_request_info info = { .cmd = cmd, .flags = 0 };
	struct net_device *net = (struct net_device *)netif->state;
	int ret;
...
module_driver(nlsock, nlsock_init, NULL, 0)
