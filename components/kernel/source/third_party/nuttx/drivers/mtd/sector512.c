  ...
  if (priv)
    {
      ...
      priv->mtd.type = mtd->type;
      priv->mtd.size = mtd->size;
      priv->mtd.erasesize = mtd->erasesize;
      priv->mtd.writesize = mtd->writesize;
      priv->mtd.oobsize = mtd->oobsize;
      priv->mtd.oobavail = mtd->oobavail;
      priv->mtd.ecc_strength = mtd->ecc_strength;
