  ...
  if (rwb->wrnblocks > 0)
    {
      ...
      /* 5. We update a portion at the beginning of the write buffer */
      ...
      else /* if (rwb->wrblockstart >= startblock && wrbend >= newend) */
        {
          ...
          if (ncopy > (size_t)rwb->wrmaxblocks - rwb->wrnblocks)
            {
...
static int rwb_rhreload(FAR struct rwbuffer_s *rwb, off_t startblock)
{
  ...
  if (startblock >= (off_t)rwb->nblocks)
    {
      return -ESPIPE;
    }
  ...
  if (rwb->wrmaxblocks > 0)
    {
      ...
      if (!rwb->wrbuffer)
        {
          ferr("Write buffer kmm_malloc(%ld) failed\n", allocsize);
          return -ENOMEM;
        }
