...
static FAR struct work_notifier_entry_s *work_notifier_find(uint32_t key)
{
  ...
  for (entry  = dq_peek(&g_notifier_pending);
       ...
       entry  = dq_next(entry))
    {
      notifier = container_of(entry, struct work_notifier_entry_s, dq);
...
void __attribute__((weak)) notifier_worker_proxy(struct work_notifier_s *info)
{
  /* Left empty to avoid compile error */
}
...
static void work_notifier_worker(FAR void *arg)
{
  ...
  if (notifier->info.remote)
    {
      notifier_worker_proxy(&notifier->info);
    }
  else
    {
      if (notifier->info.worker)
        notifier->info.worker(notifier->info.arg);

      if (notifier->info.worker2)
        notifier->info.worker2(notifier->info.arg, notifier->info.param);
    }
  ...
  if (notifier->info.oneshot)
    dq_addlast((FAR dq_entry_t *)&notifier->dq, &g_notifier_free);
...
{
  FAR struct work_notifier_entry_s *notifier;
  struct dq_entry_s *pdq;
  struct dq_entry_s *entry;
  int ret;

  DEBUGASSERT(info != NULL && (info->worker != NULL || info->worker2 != NULL));
  ...
  for (entry  = dq_peek(&g_notifier_free);
       entry != NULL;
       entry  = dq_next(entry))
    {
      notifier = container_of(entry, struct work_notifier_entry_s, dq);
      if (work_available(&notifier->work))
        {
          dq_rem((FAR dq_entry_t *)&notifier->dq, &g_notifier_free);
          pdq = entry;
          break;
        }
    }
  ...
  if (pdq == NULL)
    {
      ...
      notifier = malloc(sizeof(struct work_notifier_entry_s));
    }
  else
    {
      notifier = container_of(pdq, struct work_notifier_entry_s, dq);
    }
  ...
  if (notifier == NULL)
  ...
  else
    {
      ...
      dq_addlast((FAR dq_entry_t *)&notifier->dq, &g_notifier_pending);
      ret = notifier->key;
...
int work_notifier_teardown(int key)
{
  ...
  if (notifier == NULL)
  ...
  else
    {
      ...
      dq_rem((FAR dq_entry_t *)&notifier->dq, &g_notifier_pending);
...
void work_notifier_signal(unsigned int evtype,
                          FAR void *qualifier)
{
  work_notifier_signal2(evtype, qualifier, 0);
}

void work_notifier_signal2(unsigned int evtype,
                          FAR void *qualifier, unsigned long param)
{
  FAR struct work_notifier_entry_s *notifier;
  FAR dq_entry_t *entry;
  FAR dq_entry_t *next;
      ...
      if (info->evtype == evtype && info->qualifier == qualifier)
        {
          ...
          if (notifier->info.oneshot)
            {
              dq_rem((FAR dq_entry_t *)entry, &g_notifier_pending);
            }
          ...
          work_queue(info->qid, &notifier->work,
                     work_notifier_worker, notifier, 0);
        }
    }
