                               ...
                               FAR void *buffer, lfs_size_t size)
{
  ...
  if (INODE_IS_MTD(drv))
    {
      ret = MTD_BREAD(drv->u.i_mtd, (off_t)block, size, buffer);
    }
  else
    {
      ret = drv->u.i_bops->read(drv, buffer, (blkcnt_t)block, size);
    }
                                ...
                                FAR const void *buffer, lfs_size_t size)
{
  ...
  if (INODE_IS_MTD(drv))
    {
      ret = MTD_BWRITE(drv->u.i_mtd, (off_t)block, size, buffer);
    }
  else
    {
      ret = drv->u.i_bops->write(drv, buffer, (blkcnt_t)block, size);
    }
