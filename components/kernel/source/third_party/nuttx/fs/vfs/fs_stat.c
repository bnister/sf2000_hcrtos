...
int stat_recursive(FAR const char *path,
                          FAR struct stat *buf, int resolve)
{
  ...
  if (INODE_IS_MOUNTPT(inode))
    {
       ...
       */
      // printf("\t==>(stat_recursive -- line:%u) inode:%p\n", __LINE__, inode);
...
int nx_stat(FAR const char *path, FAR struct stat *buf, int resolve)
{
  ...
  buf->st_count = 0;
#endif
  ret = stat_recursive(path, buf, resolve);
  return ret;
}
  ...
  if (INODE_IS_MTD(inode))
    {
      ...
      if (inode->u.i_mtd != NULL &&
        ...
        {
          buf->st_size = (uint64_t)mtdgeo.neraseblocks * mtdgeo.erasesize;
        }
    }
  else
#endif
          ...
          if ((inode->u.i_bops != NULL) &&
              (inode->u.i_bops->geometry != NULL))
            {
              ...
              if (inode->u.i_bops->geometry(inode, &geo) >= 0 &&
                  geo.geo_available)
                {
                  buf->st_size = (uint64_t)geo.geo_nsectors * geo.geo_sectorsize;
                }
            }
#endif
        }
