...
#define NO_HOLDER ((TaskHandle_t)NULL)
...
struct inode_sem_s
{
  SemaphoreHandle_t   sem;     /* The semaphore */
  TaskHandle_t   holder;  /* The current holder of the semaphore */
...
void inode_initialize(void)
{
  ...
  g_inode_sem.sem = xSemaphoreCreateMutex();
...
int inode_semtake(void)
{
  ...
  /* Take the semaphore (perhaps waiting) */
    ...
    {
      xSemaphoreTake(g_inode_sem.sem, portMAX_DELAY);
...
{
  DEBUGASSERT(g_inode_sem.holder == xTaskGetCurrentTaskHandle());
  ...
  /* Yes.. then we can really release the semaphore */
  ...
  else
    {
      g_inode_sem.holder = NO_HOLDER;
      g_inode_sem.count  = 0;
      xSemaphoreGive(g_inode_sem.sem);
    }
}
