...
static int _files_semtake(FAR struct filelist *list)
{
  if (!uxInterruptNesting && !vTaskIsInCritical())
    return xSemaphoreTake(list->fl_sem, portMAX_DELAY);
  else
    return 0;
}
...
static int _files_semgive(FAR struct filelist *list)
{
  if (!uxInterruptNesting && !vTaskIsInCritical())
    return xSemaphoreGive(list->fl_sem);
  else
    return 0;
}
...
static int files_extend(FAR struct filelist *list, size_t row)
{
  ...
  tmp = realloc(list->fl_files, sizeof(FAR struct file *) * row);
  DEBUGASSERT(tmp);
  if (tmp == NULL)
    {
      return -ENFILE;
  ...
  do
    {
      tmp[i] = malloc(sizeof(struct file) *
      ...
      if (tmp[i] == NULL)
        ...
        {
          memset(tmp[i], 0, sizeof(struct file) * CONFIG_NFILE_DESCRIPTORS_PER_BLOCK);
        }
    }
  while (++i < row);
...
void files_initlist(FAR struct filelist *list)
{
  ...
  list->fl_sem = xSemaphoreCreateMutex();
}
