...
static SemaphoreHandle_t g_devno_sem = NULL;
...
static FAR char *unique_chardev(void)
{
  ...
  if (g_devno_sem == NULL)
    g_devno_sem = xSemaphoreCreateMutex();
  ...
  for (; ; )
    {
      ...
      ret = xSemaphoreTake(g_devno_sem, portMAX_DELAY);
      if (ret < 0)
        {
          return NULL;
        }
