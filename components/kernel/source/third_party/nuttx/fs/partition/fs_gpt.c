                         ...
                         FAR struct gpt_header_s *gpt)
{
  ...
  crc = nuttx_crc32part((FAR const uint8_t *)pte, size, ~0l) ^ ~0l;
                               ...
                               FAR struct gpt_header_s *gpt, blkcnt_t lba)
{
  ...
  gpt->header_crc32 = 0;
  crc = nuttx_crc32part((FAR const uint8_t *)gpt,
                  le32toh(gpt->header_size), ~0l) ^ ~0l;
  if (crc != le32toh(origcrc))
    {
      ferr("GUID Partition Table Header CRC is wrong: %lx != %lx\n", crc, le32toh(origcrc));
      return -EINVAL;
    }
  ...
  if (le64toh(gpt->my_lba) != (uint64_t)lba)
    {
      ferr("GPT: my_lba incorrect: %lld != %lld\n",
  ...
  if (le64toh(gpt->last_usable_lba) > lastlba)
    {
      if (REG32_GET_FIELD2((uint32_t)&MSYSIO0 + 0x0, 16, 16) == 0x1600) {
         ...
         *         差值在 10MB以内, 就视为正常
         */
        if(le64toh(gpt->last_usable_lba) - lastlba > (10 * 1024 * 1024)){
          ferr("GPT: last_usable_lba incorrect: %lld > %lld\n",
              le64toh(gpt->last_usable_lba), lastlba);
          return -EINVAL;
        }
