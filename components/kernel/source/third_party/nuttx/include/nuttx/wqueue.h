...
#    ifdef CONFIG_SCHED_HPWORK
#      define CONFIG_SCHED_LPWORKPRIOMAX (CONFIG_SCHED_HPWORKPRIORITY-1)
#    else
...
#  define RPCWORK  USRWORK

#else
/* Kernel mode */

#  define RPCWORK  ( 1 << 0)  /* High priority, kernel-mode rpc queue */
#  define HPWORK   ( 1 << 1 ) /* High priority, kernel-mode work queue */
#  if defined(CONFIG_SCHED_LPWORK) && defined(CONFIG_SCHED_HPWORK)
#    define LPWORK ( 1 << 2 ) /* Low priority, kernel-mode work queue */
#  else
#    define LPWORK ( 1 << 2)  /* Redirect low-priority references */
#  endif
#  define USRWORK  ( 1 << 3)  /* Redirect user-mode references */
...
struct kworker_s
{
  TaskHandle_t pid;    /* The task ID of the worker thread */
...
{
  unsigned int evtype;      /* Any event type */
  uint8_t qid;         /* The work queue to use: HPWORK or LPWORK */
  uint8_t remote;      /* The notifier_worker is from local or remote AMP CPU */
  uint8_t oneshot;      /* The oneshot event or contnuous event */
  void *qualifier; /* Event qualifier value */
  void *arg;       /* User-defined worker function argument */
  unsigned long param;     /* parameters of event pass to notifier worker function */
  worker_t worker;     /* The worker function to schedule */
  worker2_t worker2;     /* The notifier worker function to schedule */
};
 ...
 ****************************************************************************/
struct kwork_wqueue_s;
int work_qcancel(FAR struct kwork_wqueue_s *wqueue,
