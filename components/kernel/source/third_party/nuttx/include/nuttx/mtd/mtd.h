...
#define MTDIOC_GEOMETRY   _IOWR(MTD_IOCBASE, 0x0001, struct mtd_geometry_s) /* IN:  Pointer to write-able struct
...
#define MTDIOC_BULKERASE  _IO  (MTD_IOCBASE, 0x0003) /* IN:  None
                                           * OUT: None */
#define MTDIOC_PROTECT    _IOW (MTD_IOCBASE, 0x0004, struct mtd_protects_s) /* IN:  Pointer to read-able struct
                                           *      mtd_protects_s that provides
                                           *      the region to protect.
                                           * OUT: None */
#define MTDIOC_UNPROTECT  _IOW (MTD_IOCBASE, 0x0005, struct mtd_protects_s) /* IN:  Pointer to read-able struct
                                           *      mtd_protects_s that provides
                                           *      the region to un-protect.
                                           * OUT: None */
#define MTDIOC_SETSPEED   _IO  (MTD_IOCBASE, 0x0006) /* IN:  New bus speed in Hz
                                           * OUT: None */
#define MTDIOC_EXTENDED   _IO  (MTD_IOCBASE, 0x0007) /* IN:  unsigned long
                                           *      0=Use normal memory region
                                           *      1=Use alternate/extended memory
                                           * OUT: None */
#define MTDIOC_ECCSTATUS  _IOWR(MTD_IOCBASE, 0x0008, uint8_t) /* IN:  Pointer to uint8_t
                                           * OUT: ECC status */
#define MTDIOC_ERASESTATE _IOWR(MTD_IOCBASE, 0x000a, uint8_t) /* IN:  Pointer to uint8_t
                                           * OUT: Byte value that represents the
                                           *      erased state of the MTD cell */
#define MTDIOC_MEMERASE   _IOW (MTD_IOCBASE, 0x000b, struct mtd_eraseinfo_s)
...
#define MTD_BLOCK_ISBAD(d, s)              ((d)->block_isbad ? (d)->block_isbad(d, s) : (-ENOSYS))
#define MTD_BLOCK_MARKBAD(d, s)            ((d)->block_markbad ? (d)->block_markbad(d, s) : (-ENOSYS))
#define MTD_READ_OOB(d, b, da, dl, o, ol)  ((d)->read_oob ? (d)->read_oob(d, b, da, dl, o, ol) : (-ENOSYS))
#define MTD_WRITE_OOB(d, b, da, dl, o, ol) ((d)->write_oob ? (d)->write_oob(d, b, da, dl, o, ol) : (-ENOSYS))
#define MTD_CAN_HAVE_BB(d) (!!(d)->block_isbad)
...
struct mtd_eraseinfo_s
{
  uint32_t start;
  uint32_t length;     /* must be a multiple of blocksize. */
};
...
struct mtd_dev_s
{
  ...
  int (*write_oob)(FAR struct mtd_dev_s *dev, off_t offset,
                    const uint8_t *data, int data_len,
		    const uint8_t *oob, int oob_len);
  int (*read_oob)(FAR struct mtd_dev_s *dev, off_t offset,
                    uint8_t *data, int data_len,
		    uint8_t *oob, int oob_len);

  int (*block_markbad)(FAR struct mtd_dev_s *dev, off_t block_no);

  int (*block_isbad)(FAR struct mtd_dev_s *dev, off_t block_no);
