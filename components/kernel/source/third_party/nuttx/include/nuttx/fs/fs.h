...
struct file_operations
{
  ...
  int     (*poll)(FAR struct file *filep, poll_table *wait);
...
struct filelist
{
  SemaphoreHandle_t             fl_sem;     /* Manage access to the file list */
...
struct file_struct
{
  ...
  SemaphoreHandle_t                   fs_sem;       /* For thread safety */
  TaskHandle_t                   fs_holder;    /* Holder of sem */
...
struct streamlist
{
  SemaphoreHandle_t                   sl_sem;   /* For thread safety */
...
int dummy_open(struct file *file);
int dummy_close(struct file *file);
ssize_t dummy_read(FAR struct file *file , FAR char *buffer , size_t buflen);
ssize_t dummy_write(FAR struct file *file , FAR const char *buffer ,
		size_t buflen);
...
int fs_getfilefd(FAR struct file *filep);
...
//int file_poll(FAR struct file *filep, poll_table *wait);
