...
static const struct flash_info *
spi_nor_search_general_part_by_id(const struct flash_info *parts, unsigned int nparts,
			  const u8 *id)
{
	unsigned int i;
...
static const struct flash_info *spi_nor_read_id(struct spi_nor *nor)
{
	...
	elog_hexdump2(ELOG_LVL_ERROR, "JEDEC id bytes:", 16, id, SPI_NOR_MAX_ID_LEN);
	...
	elog_hexdump("JEDEC id bytes:", 16, id, SPI_NOR_MAX_ID_LEN);
	...
	elog_hexdump2(ELOG_LVL_ERROR, "This id is no in flash id table ,trying use general id:", 16, id, SPI_NOR_MAX_ID_LEN);
	info = spi_nor_search_general_part_by_id(spi_nor_unknown.parts,
				spi_nor_unknown.nparts,
				 id);
	if (info) {
	       nor->manufacturer = &spi_nor_unknown;
	       return info;
	}
#endif
...
static int spi_nor_create_read_dirmap(struct spi_nor *nor)
{
	...
	nor->dirmap.rdesc = spi_mem_dirmap_create(nor->spimem,
						       &info);
...
static int spi_nor_create_write_dirmap(struct spi_nor *nor)
{
	...
	nor->dirmap.wdesc = spi_mem_dirmap_create(nor->spimem,
						       &info);
	return PTR_ERR_OR_ZERO(nor->dirmap.wdesc);
}

int spi_nor_probe(struct spi_mem *spimem, const char *node)
{
	...
	return mtd_device_register(&nor->mtd, node);
}
