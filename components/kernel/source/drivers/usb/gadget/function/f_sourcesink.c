...
{
	__attribute__((unused)) struct usb_composite_dev *cdev = c->cdev;
...
static int check_read_data(struct f_sourcesink *ss, struct usb_request *req)
{
	unsigned		i;
	u8			*buf = req->buf;
	__attribute__((unused)) struct usb_composite_dev *cdev = ss->function.config->cdev;
		...
		unsigned intf, unsigned alt)
{
	...
	// disable_source_sink(ss); //tony.su debug
...
static struct usb_function_instance *source_sink_alloc_inst(void)
{
	...
	pr_info("--> usb gadget : alloc SourceSink funtions\n");
