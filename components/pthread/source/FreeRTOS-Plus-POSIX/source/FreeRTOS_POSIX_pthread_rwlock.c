...
int pthread_rwlock_destroy(pthread_rwlock_t *rwlock)
{
	...
	if (rwlock->rw_refcount != 0 || rwlock->rw_nwaitreaders != 0 || rwlock->rw_nwaitwriters != 0) {
		result = EBUSY;
		return result;
	} else {
		/* check whether busy */
		result = xSemaphoreTake(( SemaphoreHandle_t )&(rwlock->rw_condreaders.xCondWaitSemaphore), 0);
		if (result == pdPASS) {
			result = xSemaphoreTake((SemaphoreHandle_t)&(rwlock->rw_condwriters.xCondWaitSemaphore), 0);
			if (result == pdPASS) {
				xSemaphoreGive((SemaphoreHandle_t)&(rwlock->rw_condreaders.xCondWaitSemaphore));
				xSemaphoreGive((SemaphoreHandle_t)&(rwlock->rw_condwriters.xCondWaitSemaphore));
