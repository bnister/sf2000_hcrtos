...
#
################################################################################
ifeq ($(BR2_PACKAGE_WPA_SUPPLICANT_RTL_V0_8),y)
include $(TOPDIR)/components/hclib/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/version_0_8.mk
else ifeq ($(BR2_PACKAGE_WPA_SUPPLICANT_RTL_V2_7),y)
include $(TOPDIR)/components/hclib/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/version_2_7.mk
else
$(error not configure wpa_supplicant supported version) 
endif
WPA_SUPPLICANT_SITE_METHOD = git
WPA_SUPPLICANT_LICENSE = LGPL-2.0+
WPA_SUPPLICANT_LICENSE_FILES = COPYING
WPA_SUPPLICANT_CONF_OPTS = 

WPA_SUPPLICANT_CONFIG = $(WPA_SUPPLICANT_DIR)/wpa_supplicant/.config
WPA_SUPPLICANT_SUBDIR = wpa_supplicant
WPA_SUPPLICANT_DBUS_SERVICE = fi.w1.wpa_supplicant1
WPA_SUPPLICANT_CFLAGS = $(TARGET_CFLAGS) -D__linux__ -D__HCRTOS__ #-D__DragonFly__  #-I$(STAGING_DIR)/usr/include/libnl3/
WPA_SUPPLICANT_LDFLAGS = $(TARGET_LDFLAGS)
WPA_SUPPLICANT_SELINUX_MODULES = networkmanager

WPA_SUPPLICANT_DEPENDENCIES = kernel

WPA_SUPPLICANT_EXTRACT_CMDS = \
	git -C $(@D) init && \
	git -C $(@D) remote add origin $(WPA_SUPPLICANT_SITE) && \
	git -C $(@D) fetch && \
	git -C $(@D) checkout master && \
	git -C $(@D) checkout $(WPA_SUPPLICANT_VERSION)
...
ifeq ($(BR2_PACKAGE_LIBOPENSSL),y_y)
...
define WPA_SUPPLICANT_BUILD_CMDS
	#rsync -au --chmod=u=rwX,go=rX  --exclude .svn --exclude .git --exclude .hg --exclude .bzr --exclude CVS --delete-excluded $(TOPDIR)/components/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/source/ $(@D)
	cd $(@D);find ./ -name "*.o"|xargs rm -rf
	$(TARGET_MAKE_ENV) CFLAGS="$(WPA_SUPPLICANT_CFLAGS)" \
		...
		$(MAKE) CC="$(TARGET_CC)" -C $(@D)/$(WPA_SUPPLICANT_SUBDIR) all V=1
endef
...
define WPA_SUPPLICANT_CLEAN_CMDS
	$(TARGET_MAKE_ENV) CFLAGS="$(WPA_SUPPLICANT_CFLAGS)" \
		...
		LIBS="$(WPA_SUPPLICANT_LIBS)" LIBS_c="$(WPA_SUPPLICANT_LIBS)" \
		LIBS_p="$(WPA_SUPPLICANT_LIBS)" \
		$(MAKE) CC="$(TARGET_CC)" -C $(@D)/$(WPA_SUPPLICANT_SUBDIR) clean V=1
endef
...
define WPA_SUPPLICANT_INSTALL_CLI
	...
	$(INSTALL) -m 0755 -D $(@D)/hostapd/hostapd_cli \
...
define WPA_SUPPLICANT_INSTALL_STAGING_CMDS
	...
	$(INSTALL) -m 0644 -D $(@D)/src/common/wpa_ctrl.h \
		$(PREBUILT_DIR)/usr/include/wpa_ctrl.h
endef

define WPA_SUPPLICANT_INSTALL_TARGET_CMDS
	$(INSTALL) -m 0755 -D $(@D)/$(WPA_SUPPLICANT_SUBDIR)/libwpa_supplicant.a \
		$(PREBUILT_DIR)/usr/lib/libwpa_supplicant.a
#	$(INSTALL) -m 0755 -D $(@D)/hostapd/libhostapd.a\
#		$(PREBUILT_DIR)/usr/lib/libhostapd.a

	#$(INSTALL) -m 644 -D ../package/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/wpa_supplicant.conf \
		$(TARGET_DIR)/etc/wpa_supplicant.conf
	#$(INSTALL) -m 644 -D ../package/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/hostapd.conf \
		$(TARGET_DIR)/etc/hostapd.conf
	#$(INSTALL) -m 644 -D ../package/wpa_supplicant/wpa_supplicant-rtl/wpa_supplicant/hostapd_5G.conf \
		$(TARGET_DIR)/etc/hostapd_5G.conf
