...
FFMPEG_OVERRIDE_SRCDIR = $(TOPDIR)/components/ffmpeg/source
...
ffmpeg : ffmpeg-configure

ffmpeg-configure: | $(BUILD_DIR)/ffmpeg-custom/.stamp_patched

$(BUILD_DIR)/ffmpeg-custom/.stamp_patched: | $(BUILD_DIR)/ffmpeg-custom/.stamp_rsynced
...
ifeq ($(BR2_PACKAGE_LIBOPENSSL)$(CONFIG_NET),yy)
...
FFMPEG_CFLAGS += -Wall -Wno-unused-parameter -Wno-undef -Wno-sign-compare -Wno-implicit-fallthrough \
	-Wno-incompatible-pointer-types -Wno-missing-field-initializers -Wno-unused-function \
	-Wno-redundant-decls -Wno-unused-variable -Wno-format-truncation -Wno-discarded-qualifiers \
	-Wno-old-style-declaration -Wno-format-nonliteral -Wno-format -DCONFIG_PLATFORM_HICHIP

FFMPEG_CONF_ENV += CFLAGS="$(FFMPEG_CFLAGS)"
FFMPEG_CONF_OPTS += $(call qstrip,$(BR2_PACKAGE_FFMPEG_EXTRACONF))

ifeq ($(BR2_PACKAGE_FFMPEG_GDB_DBG),y)
FFMPEG_CONF_OPTS += $(call qstrip," --enable-debug --disable-optimizations --disable-asm --disable-stripping ")
endif
...
define FFMPEG_REMOVE_STAGING_STDATOMIC_H
	if test -f $(STAGING_DIR)/usr/include/stdatomic.h; then \
		mv $(STAGING_DIR)/usr/include/stdatomic.h $(STAGING_DIR)/usr/include/stdatomic.h.bak; \
	fi;
endef

define FFMPEG_FIX_HAVE_STRUCT_ADDRINFO
	sed -i 's/HAVE_STRUCT_ADDRINFO 0/HAVE_STRUCT_ADDRINFO 1/g' $(@D)/config.h
	sed -i 's/HAVE_INET_ATON 0/HAVE_INET_ATON 1/g' $(@D)/config.h
endef

define FFMPEG_RESTORE_STAGING_STDATOMIC_H
	if test -f $(STAGING_DIR)/usr/include/stdatomic.h.bak; then \
		mv $(STAGING_DIR)/usr/include/stdatomic.h.bak $(STAGING_DIR)/usr/include/stdatomic.h; \
	fi;
endef

FFMPEG_PRE_CONFIGURE_HOOKS += FFMPEG_REMOVE_STAGING_STDATOMIC_H
FFMPEG_PRE_BUILD_HOOKS += FFMPEG_REMOVE_STAGING_STDATOMIC_H FFMPEG_FIX_HAVE_STRUCT_ADDRINFO
FFMPEG_POST_BUILD_HOOKS += FFMPEG_RESTORE_STAGING_STDATOMIC_H

FFMPEG_EXTRACT_CMDS = \
	git -C $(@D) init && \
	git -C $(@D) remote add origin $(FFMPEG_SITE) && \
	git -C $(@D) fetch && \
	git -C $(@D) checkout master && \
	git -C $(@D) checkout $(FFMPEG_VERSION)
