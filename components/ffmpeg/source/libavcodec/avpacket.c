...
int av_packet_from_mmz_data(AVPacket *pkt, uint8_t *user_addr, uint32_t kernel_addr,
    int size, void (*free)(void *opaque, uint8_t *user_addr), void *opaque)
{
    if (size >= INT_MAX - AV_INPUT_BUFFER_PADDING_SIZE)
        return AVERROR(EINVAL);

    pkt->buf = av_buffer_create(user_addr, size + AV_INPUT_BUFFER_PADDING_SIZE,
                                free, opaque, 0);
    if (!pkt->buf)
        return AVERROR(ENOMEM);

    pkt->data = user_addr;
    pkt->kernel_addr = kernel_addr;
    pkt->size = size;
