                                     ...
                                     int channel_config)
{
    ...
    m4ac->sbr = 0;
#if USE_FIXED
    if ((m4ac->frame_length_short = get_bits1(gb))) { // frameLengthFlag
        if (m4ac->object_type != AOT_ER_AAC_LD && m4ac->object_type != AOT_ER_AAC_ELD) {
...
static av_cold int aac_decode_init(AVCodecContext *avctx)
{
    ...
    ret = AAC_RENAME_32(ff_mdct15_init)(&ac->mdct120, 1, 3, 1.0f/(16*1024*120*2));
    if (ret < 0)
        return ret;
    ret = AAC_RENAME_32(ff_mdct15_init)(&ac->mdct480, 1, 5, 1.0f/(16*1024*960));
    if (ret < 0)
        return ret;
    ret = AAC_RENAME_32(ff_mdct15_init)(&ac->mdct960, 1, 6, 1.0f/(16*1024*960*2));
    if (ret < 0)
        return ret;
    ...
    for (type = 3; type >= 0; type--) {
        for (i = 0; i < MAX_ELEM_ID; i++) {
            ...
            if (che && che->present) {
                ...
                av_log(ac->avctx, AV_LOG_DEBUG, "ChannelElement %d.%d missing \n", type, i);
            }
        }
    }
}
