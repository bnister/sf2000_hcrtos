...
#if FFT_FLOAT
#define CMULSUB(dst, a, b, c, d)                                               \
	do {                                                                   \
		(dst) = (a) * (b) - (c) * (d);                                 \
	} while (0)
#define CMULADD(dst, a, b, c, d)                                               \
	do {                                                                   \
		(dst) = (a) * (b) + (c) * (d);                                 \
	} while (0)
#else
...
#endif
    CMULSUB(t[4].re, exptab[0].re, t[2].re, exptab[1].re, t[0].re);
    CMULSUB(t[4].im, exptab[0].re, t[2].im, exptab[1].re, t[0].im);
    CMULSUB(t[0].re, exptab[0].re, t[0].re, exptab[1].re, t[2].re);
    CMULSUB(t[0].im, exptab[0].re, t[0].im, exptab[1].re, t[2].im);
    CMULSUB(t[5].re, exptab[0].im, t[3].re, exptab[1].im, t[1].re);
    CMULSUB(t[5].im, exptab[0].im, t[3].im, exptab[1].im, t[1].im);
    CMULADD(t[1].re, exptab[0].im, t[1].re, exptab[1].im, t[3].re);
    CMULADD(t[1].im, exptab[0].im, t[1].im, exptab[1].im, t[3].im);
...
static void mdct15(MDCT15Context *s, FFTSample *dst, const FFTSample *src, ptrdiff_t stride)
{
    int i, j;
...
static void imdct15_half(MDCT15Context *s, FFTSample *dst, const FFTSample *src,
                         ptrdiff_t stride)
{
    FFTComplex fft15in[15];
    FFTComplex *z = (FFTComplex *)dst;
    int i, j, len8 = s->len4 >> 1, l_ptwo = 1 << s->ptwo_fft.nbits;
    const FFTSample *in1 = src, *in2 = src + (s->len2 - 1) * stride;
...
av_cold int ff_mdct15_init(MDCT15Context **ps, int inverse, int N, double scale)
{
...
#if !FFT_FLOAT
        s->twiddle_exptab[i].re = lrint(cosf(alpha) * 2147483648.0);
        s->twiddle_exptab[i].im = lrint(sinf(alpha) * 2147483648.0);
#else
    ...
    for (i = 0; i < 19; i++) {
        if (i < 15) {
            ...
            if (!s->inverse)
                theta *= -1;
#if !FFT_FLOAT
            s->exptab[i].re = lrint(cosf(theta) * 2147483647.0);
            s->exptab[i].im = lrint(sinf(theta) * 2147483647.0);
#else
    ...
    /* 5-point FFT exptab */
#if !FFT_FLOAT
    s->exptab[19].re = lrint(cosf(2.0f * M_PI / 5.0f) * 2147483647.0);
    s->exptab[19].im = lrint(sinf(2.0f * M_PI / 5.0f) * 2147483647.0);
    s->exptab[20].re = lrint(cosf(1.0f * M_PI / 5.0f) * 2147483647.0);
    s->exptab[20].im = lrint(sinf(1.0f * M_PI / 5.0f) * 2147483647.0);
#else
