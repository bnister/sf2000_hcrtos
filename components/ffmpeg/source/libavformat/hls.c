...
typedef struct HLSContext {
    ...
    int64_t first_timestamp_offset;
                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    int64_t discount_time = 0;
    int discount_seg = 0;
    int yt_m3u8 = 0;
    char *yt_base_url = NULL;
    char *yt_prefix = NULL;
    char *yt_param = NULL;
    char *yt_params[MAX_PARAM_LEN];
    int yt_param_num = 0;
    int list_from_mem = in ? 1 : 0;
    ...
    while ((list_from_mem && !in->eof_reached) || (!list_from_mem && !avio_feof (in))) {
        ff_get_chomp_line(in, line, sizeof(line));
        //printf("line %s\n", line);
        if (av_strstart(line, "#YT-EXT-CONDENSED-URL:", &ptr)) {
            char *yt_str;
            av_log(c->ctx, AV_LOG_VERBOSE, "ptr %s\n", ptr);

            if (yt_m3u8) {
                av_log(c->ctx, AV_LOG_ERROR, "only support one ext condensed url");
                continue;
            }

            yt_m3u8 = 1;
            yt_str = strstr(ptr,"BASE-URI=");
            if (yt_str) {
                yt_base_url = malloc(strlen(yt_str));
                if (!yt_base_url) {
                    continue;
                }
                sscanf(yt_str, "%*[^\"]\"%[^\"]", yt_base_url);
                av_log(c->ctx, AV_LOG_VERBOSE, "yt_base_url %s \n", yt_base_url);
            }
            yt_str = strstr(ptr,"PARAMS=");
            if (yt_str) {
                char *saveptr = NULL;
                yt_param = malloc(strlen(yt_str));
                if (!yt_param) {
                    continue;
                }
                sscanf(yt_str, "%*[^\"]\"%[^\"]", yt_param);
                for (int i = 0; i < MAX_PARAM_LEN; i++) {
                    if (!(yt_params[i] = av_strtok(yt_param, ",", &saveptr))) {
                        break;
                    }
                    yt_param = NULL;
                    av_log(c->ctx, AV_LOG_VERBOSE, "yt_params[%d]: %s\n", i, yt_params[i]);
                    yt_param_num++;
                }
            }
            yt_str = strstr(ptr,"PREFIX=");
            if (yt_str) {
                yt_prefix = malloc(strlen(yt_str));
                if (!yt_prefix) {
                    continue;
                }
                sscanf(yt_str, "%*[^\"]\"%[^\"]", yt_prefix);
                av_log(c->ctx, AV_LOG_VERBOSE, "yt_prefix %s \n", yt_prefix);
            }
            ...
            av_log(c->ctx, AV_LOG_WARNING, "get  DISCONTINUITY\n");
            pls->has_discont_seg = 1;//for seek, if this flag on, do not drop frame by pts after seek.
            if ((discount_time <= 60 * AV_TIME_BASE) && discount_seg) {
                av_log(c->ctx, AV_LOG_FATAL, "Find a discount seg %d, probablely an advertisement, remove it."
                    "Fix me if it is not an advertisement!!!\n", pls->n_segments);
                for (int i = 0; i < discount_seg; i++) {
                    av_freep(&pls->segments[pls->n_segments - 1]->key);
                    av_freep(&pls->segments[pls->n_segments - 1]->url);
                    av_freep(&pls->segments[pls->n_segments - 1]);
                    pls->n_segments--;
                    av_log(c->ctx, AV_LOG_WARNING, "pls->segment--\n");
                }
            }
            discount_time = 0;
            discount_seg = 0;
            ...
            if (is_segment) {
                ...
                if (yt_m3u8 && yt_prefix && av_strstart(line, yt_prefix, &ptr)) {
                    char *suffix, *suffix_tmp, *arg, *saveptr = NULL;

                    memset(tmp_str, 0, MAX_URL_SIZE);
                    strcpy(tmp_str, yt_base_url);

                    if (ptr) {
                        suffix_tmp = suffix = strdup(ptr);
                        if (!suffix) {
                            continue;
                        }

                        av_log(c->ctx, AV_LOG_TRACE, "suffix %s\n", suffix);
                        for (int i = 0; i < yt_param_num; i++) {
                            if (!(arg = av_strtok(suffix_tmp, "/", &saveptr))) {
                                break;
                            }
                            suffix_tmp = NULL;
                            av_log(c->ctx, AV_LOG_TRACE, "yt_params[%d]: %s, arg: %s\n", i, yt_params[i], arg);
                            strcat(tmp_str,"/");
                            strcat(tmp_str,yt_params[i]);
                            strcat(tmp_str,"/");
                            strcat(tmp_str,arg);
                        }
                        free(suffix);
                ...
                discount_time += duration;
                discount_seg++;
                av_log(c->ctx, AV_LOG_TRACE, "discount_seg %d, discount_time %lld\n", discount_seg, discount_time);
                is_segment = 0;
...
static void parse_id3(AVFormatContext *s, AVIOContext *pb,
                      AVDictionary **metadata, int64_t *pts,
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg, AVIOContext **in)
{
    ...
    av_dict_set(&opts, "read_seek", "1", 0);
...
reload:
        ...
        if (!v->finished &&
            ...
            if (v->yt_m3u8) {
                ret = parse_playlist(c, v->parent->url, v, v->parent->pb);
            } else {
                ret = parse_playlist(c, v->url, v, NULL);
            }
            if (ret < 0) {
                if (ret != AVERROR_EXIT)
        ...
        if (ret < 0) {
            ...
            if (c->first_timestamp == AV_NOPTS_VALUE &&
                (v->cur_seq_no - v->start_seq_no) < c->playlists[0]->n_segments) {
                c->first_timestamp_offset += c->playlists[0]->segments[v->cur_seq_no - v->start_seq_no]->duration;
                av_log(v->parent, AV_LOG_WARNING, "c->first_timestamp_offset1 %lld\n", c->first_timestamp_offset);
            }
            v->cur_seq_no += 1;
            goto reload;
        }
...
static int find_timestamp_in_playlist(HLSContext *c, struct playlist *pls,
    int64_t timestamp, int64_t *seq_no, int64_t *seq_start_timestamp)
{
    int i;
    ...
    if (timestamp < pos) {
        *seq_no = pls->start_seq_no;
        if (seq_start_timestamp) {
            *seq_start_timestamp = pos;
        }
        return 0;
    }
    ...
    *seq_no = pls->start_seq_no + pls->n_segments - 1;
    ...
    if (seq_start_timestamp) {
        *seq_start_timestamp = pos - pls->segments[pls->n_segments - 1]->duration;
    }
    return 0;
}
...
static int64_t select_cur_seq_no(HLSContext *c, struct playlist *pls)
{
    ...
    if ((pls->finished || pls->start_seq_no == 0) && c->cur_timestamp != AV_NOPTS_VALUE) {
...
static int hls_read_header(AVFormatContext *s)
{
    ...
    if (c->n_variants > 1) {
        av_dict_set_int(&s->metadata, "n_variants", c->n_variants, 0);
    }
    ...
    /* maybe a broken playlist try again */
    if (!c->variants[0]->playlists[0]->finished &&
        c->variants[0]->playlists[0]->start_seq_no == 0) {
        if (ff_check_interrupt(&s->interrupt_callback)) {
            goto fail;
        }
        av_log(s, AV_LOG_WARNING, "broken playlist, parse again\n");
        parse_playlist(c, c->variants[0]->playlists[0]->url, c->variants[0]->playlists[0], NULL);
        av_log(s, AV_LOG_WARNING, "reparsed finished flag %d\n",
            c->variants[0]->playlists[0]->finished);
    }
    ...
    if (c->variants[0]->playlists[0]->finished ||
        ...
        int64_t duration = 0;
        if (!c->variants[0]->playlists[0]->finished) {
            av_log(s, AV_LOG_WARNING, "finished %d, start_seq_no %d, maybe a living stream, maybe a VOD stream\n",
                c->variants[0]->playlists[0]->finished, c->variants[0]->playlists[0]->start_seq_no);
            c->live_start_index = 0;//we don't know playback from seq 0;
        }
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->yt_m3u8 && pls->n_init_sections) {
            //printf("pls->init_sections[0]->url: %s\n", pls->init_sections[0]->url);
            ret = avformat_open_input(&pls->ctx, pls->init_sections[0]->url, in_fmt, NULL);
        } else {
...
{
    if (pls->id3_offset >= 0) {
        pls->pkt->pts = pls->id3_mpegts_timestamp +
...
static int hls_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->needed && !pls->pkt->data) {
            while (1) {
                ...
                if (ret < 0) {
                    ...
                    if (c->first_timestamp == AV_NOPTS_VALUE &&
                        pls->pkt->pts != AV_NOPTS_VALUE) {
                        /* Y2B sync bug: if the beging segments linked err,
                         * then we should adjust the first timestamp to keep sync */
                        if ((pls->ctx->nb_streams == 1 || pls->yt_m3u8) && c->first_timestamp_offset) {
                            pls->pkt->pts -= av_rescale_q(c->first_timestamp_offset,
                                 AV_TIME_BASE_Q, get_timebase(pls));
                        }
                        c->first_timestamp = av_rescale_q(pls->pkt->pts,
                ...
                if (pls->seek_stream_index < 0 ||
                    ...
                    tb = get_timebase(pls);
                    ts_diff = av_rescale_rnd(pls->pkt->pts, AV_TIME_BASE,
                                            tb.den, AV_ROUND_DOWN) -
                            pls->seek_timestamp;
                    /* sometimes the ts_diff may have a margin of 1 microsecond,
                     * so change ts_diff >= 0 to ts_diff >= -1*/
                    if (ts_diff >= -1 && (pls->seek_flags & AVSEEK_FLAG_ANY ||
        ...
        /* Check if this stream has the packet with the lowest pts */
        if (pls->pkt->data) {
            ...
            if (minplaylist < 0) {
                ...
                int64_t pts     =    pls->pkt->pts;
                int64_t minpts  = minpls->pkt->pts;

                if (pts == AV_NOPTS_VALUE ||
                    (minpts != AV_NOPTS_VALUE && compare_ts_with_wrapdetect(pts, pls, minpts, minpls) < 0))
                    minplaylist = i;
            }
        }
    }
    ...
    if (minplaylist >= 0) {
        ...
        if (pkt->pts != AV_NOPTS_VALUE)
            c->cur_timestamp = av_rescale_q(pkt->pts,
                                            ist->time_base,
                                            AV_TIME_BASE_Q);
                               ...
                               int64_t timestamp, int flags)
{
    ...
    seg_start_time = seek_timestamp = av_rescale_rnd(timestamp, AV_TIME_BASE,
    ...
    if (!seek_pls || !find_timestamp_in_playlist(c, seek_pls, seek_timestamp, &seq_no, &seg_start_time)) {
        return AVERROR(EIO);
    }

    /* if the gap between seek time & seg start time is less than 10s, 
     * we use seg start time as real seek time, else we use origin seek time */
    if (seek_timestamp - seg_start_time > 10 * 1000 * 1000) {
        seek_timestamp_real = seek_timestamp;
    } else {
        seek_timestamp_real = seg_start_time;
    }
	
    /* set segment now so we do not need to search again below */
    seek_pls->cur_seq_no = seq_no;
    seek_pls->seek_stream_index = stream_subdemuxer_index;
    if (seek_pls->has_discont_seg) {
        seek_pls->seek_timestamp = AV_NOPTS_VALUE;
    } else {
        seek_pls->seek_timestamp = seek_timestamp_real;
    }
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->has_discont_seg) {
            pls->seek_timestamp = AV_NOPTS_VALUE;
        } else {
            pls->seek_timestamp = seek_timestamp_real;
        }
        pls->seek_flags = flags;

        if (pls != seek_pls) {
            /* set closest segment seq_no for playlists not handled above */
            find_timestamp_in_playlist(c, pls, seek_timestamp_real, &pls->cur_seq_no, NULL);
    ...
    av_log(s, AV_LOG_VERBOSE, "seek_timestamp %lld, seg_start_time %lld, seek_timestamp_real %lld\n",
        seek_timestamp, seg_start_time, seek_timestamp_real);
    c->cur_timestamp = seek_timestamp_real;
