...
#define PROBE_PACKET_MAX_BUF 2048
...
static int discard_pid(MpegTSContext *ts, unsigned int pid)
{
    ...
    for (i = 0; i < ts->stream->nb_streams; i++) {
        if(ts->stream->streams[i]->id == pid) {
            if (ts->stream->streams[i]->discard == AVDISCARD_ALL) {
                return 1;
            }
        }
    }
...
static int get_packet_size(AVFormatContext* s)
{
    ...
    while (buf_size < PROBE_PACKET_MAX_BUF && max_iterations--) {
        ...
        if (score < 3 && dvhs_score < 3 && fec_score < 3) {
            continue;
        }
...
static int mpegts_sync_check(AVFormatContext *s)
{
    MpegTSContext *ts = s->priv_data;
    AVIOContext *pb = s->pb;
    uint64_t pos = avio_tell(pb);
    int c, i = 0, end_i;
    int check_size = 2 * ts->raw_packet_size + 4;
    uint8_t *data = NULL;
    
    if (ts->raw_packet_size < TS_PACKET_SIZE) {
        return 0;
    }
    ...
    avio_read(pb, data, check_size);
    end_i = check_size - ts->raw_packet_size - 4;
    while (i < end_i) {
        if (IS_MPEGTS_HEADER (data + i) && IS_MPEGTS_HEADER (data + i + ts->raw_packet_size)){
            avio_seek(pb, -(check_size - i), SEEK_CUR);
            free(data);
            return 0;
        }
        i++;
    ...
    for (i = 0; i < ts->resync_size; i++) {
        ...
        if (c == 0x47) {
            ...
            if (new_packet_size < 0) {
                avio_seek(pb, pos + PROBE_PACKET_MAX_BUF/2, SEEK_SET);
                return AVERROR_INVALIDDATA;
            } else if (new_packet_size != ts->raw_packet_size) {
