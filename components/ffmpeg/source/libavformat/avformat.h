...
typedef struct AVInputFormat {
    ...
    int (*free_device_capabilities)(struct AVFormatContext *s, struct AVDeviceCapabilitiesQuery *caps);
#endif
    int (*sync_check)(struct AVFormatContext *);
    int (*seek_check)(struct AVFormatContext *);
} AVInputFormat;
/**
 * @}
 */
...
#if 1
//Yeah, trying to keep the size of this small to reduce memory requirements (it is 16 vs. 24 bytes due to possible 8-byte alignment).
typedef struct AVIndexEntry {
    int64_t min_distance:27;  /* max 64Mbytes *//**< Minimum distance between this and the previous keyframe, used to avoid unneeded searching. */
    int64_t pos:37;           /* max 64Gbytes */
