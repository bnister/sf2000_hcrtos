                        ...
                        int stream_index, int flush)
{
    ...
    while (size > 0 || (flush && got_output)) {
        ...
        out_pkt->kernel_addr  = pkt->kernel_addr;
        ...
        if (av_dict_count(st->parser->metadata)) {
            AVDictionaryEntry *t = NULL;
            while ((t = av_dict_get(st->parser->metadata, "", t, AV_DICT_IGNORE_SUFFIX))) {
                av_log(NULL, AV_LOG_DEBUG, "%s: ", t->key);
                av_log(NULL, AV_LOG_DEBUG, "%s\n", t->value);
                av_dict_set(&s->metadata, t->key, t->value, AV_DICT_DONT_OVERWRITE);
            }
        }
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            if (st->disposition & AV_DISPOSITION_ATTACHED_PIC)
                score -= 400;
            /* we use no video decoder, so presume have width & height */
            if (1 || st->codecpar->width && st->codecpar->height)
                score += 50;
            score+= 25;
        }
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                               ...
                               int64_t timestamp, int flags)
{
    ...
    if (s->iformat->seek_check) {
        ret = s->iformat->seek_check(s);
        if (ret != 0)
            return ret;
    }
...
static void estimate_timings_from_pts(AVFormatContext *ic, int64_t old_offset)
{
    ...
    int retry = 0;
    int max_retry_times = DURATION_MAX_RETRY;
    ...
    do {
        ...
        read_size = 0;
        for (;;) {
            if (read_size >= DURATION_MAX_READ_SIZE << (FFMIN(FFMAX(retry - 1, 0), 5)))
                break;

            if (ic->iformat->sync_check) {
                ret = ic->iformat->sync_check(ic);
                if (ret < 0) {
                    break;
                }
            }
...
static void estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    if (file_size && (ic->pb->seekable & AVIO_SEEKABLE_NORMAL) &&
        !ic->pb->eof_reached && strcmp(ic->iformat->name, "hls")) {
        int64_t pos = avio_tell(ic->pb);
        int ret = avio_seek(ic->pb, 0, SEEK_SET);
        if (ret < 0) {
            av_log(ic, AV_LOG_WARNING, "the stream can not seek by bytes\n");
            ic->pb->seekable &= (~AVIO_SEEKABLE_NORMAL);
        } else {
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE) && st->internal->request_probe <= 0) {
            ...
            if (st->parser) {
                ...
                /* parse out width & height of attached pic */
                if ((st->disposition & AV_DISPOSITION_ATTACHED_PIC) && st->attached_pic.data &&
                    st->parser && st->codecpar->codec_id == AV_CODEC_ID_MJPEG) {
                    int len;
                    len = st->parser->parser->parser_parse(st->parser, avctx, NULL, NULL,
                    ...
                    st->codecpar->pic_hw_support = st->internal->avctx->pic_hw_support;
                }
    ...
    for (;;) {
        ...
        if (!missing_streams || !*missing_streams) {
            if (i == ic->nb_streams) {
                if (!strcmp(ic->iformat->name,"wav") && st->codec_info_nb_frames < 100 &&
                    (st && st->codecpar && (st->codecpar->codec_id == AV_CODEC_ID_FIRST_AUDIO))) {
                    //we should not break here, we need more data to do re-probe.
                } else {
                    analyzed_all_streams = 1;
                    av_log(ic, AV_LOG_VERBOSE, "All info of %d streams found\n", i);
...
static char *get_dict_value(AVDictionary *dict, char *key, int flags)
{
    AVDictionaryEntry *de = NULL;
    char *value = NULL;

    de = av_dict_get(dict, key, de, flags);
    if (de) {
        value = de->value;
    }
                        ...
                        AVCodec **decoder_ret, int flags)
{
    ...
    char *n_variants_str = NULL;
    int n_variants = -1;
    
    n_variants_str = get_dict_value(ic->metadata, "n_variants", AV_DICT_MATCH_CASE);
    if (n_variants_str) {
        n_variants = atoi(n_variants_str);
    }
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        if ((n_variants > 1) && get_dict_value(st->metadata, "variant_bitrate", AV_DICT_MATCH_CASE)) {
            bitrate = atoi(get_dict_value(st->metadata, "variant_bitrate", AV_DICT_MATCH_CASE));
        } else {
            bitrate = par->bit_rate;
        }

        if (n_variants > 1 && multiframe > 0) {
            //the hls has variant bitrates
            if (best_bitrate > bitrate) {
                continue;
            }
        } else {
