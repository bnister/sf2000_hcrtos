...
static int jpeg_probe_internal(const AVProbeData *p, int *large_picture);
...
static int img_read_probe(const AVProbeData *p)
{
    ...
    if (p->filename && ff_guess_image2_codec(p->filename)) {
        if (av_filename_number_test(p->filename))
            ...
            score0 = AVPROBE_SCORE_EXTENSION;
    }

    score1 = jpeg_probe_internal(p, &large_picture);
    score1 = large_picture ? (score1 + 1) : 0;
    score2 = bmp_probe(p) + 1;
    score3 = png_probe(p) + 1;
    score4 = tiff_probe(p) + 1;

    return FFMAX(FFMAX(FFMAX(FFMAX(score0, score1), score2), score3), score4);
}
...
#if ARCH_MIPS
typedef struct ff_mmz {
	uint32_t kernel_addr;
	int size;
	int fd;
} ff_mmz;

static int ff_mmz_malloced = 0;
static void ff_mmz_free(void *opaque, uint8_t *data)
{
    ff_mmz *mmz = (ff_mmz *)opaque;
#ifdef __linux__
    if (mmz && mmz->fd >= 0) {
        av_log(NULL, AV_LOG_DEBUG, "mmz free %p, %d\n", data, mmz->fd);
        munmap(data, mmz->size);
        close(mmz->fd);
        mmz->fd = -1;
        av_free(mmz);
        ff_mmz_malloced = 0;
...
#else
    if (data) {
        av_log(NULL, AV_LOG_DEBUG, "mmz free %p\n", data);
        mmz_free(0, (void *)data);
        av_free(mmz);
    }
#endif
}

static int ff_mmz_malloc(int size, void **user_addr, void **kernel_addr)
{
#ifdef __linux__
    int ret;
    struct mmz_blk mmz = {0, 32, size};
    int fd;

    if (!user_addr || !kernel_addr || ff_mmz_malloced) {
        av_log(NULL, AV_LOG_ERROR, "err, null pointer\n");
        return -1;
    }

    fd = open("/dev/mmz", O_RDWR);
    if (fd < 0) {
        return -ENODEV;
    }
    ret = ioctl(fd, MMZ_MEMALIGN, &mmz);
    if (ret < 0) {
        close(fd);
        return -ENOMEM;
    }

    *user_addr = mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    if (*user_addr == MAP_FAILED) {
        close(fd);
        return (int)MAP_FAILED;
    }

    *kernel_addr = (void *)mmz.addr;
    ff_mmz_malloced = 1;
    av_log(NULL, AV_LOG_DEBUG, "mmz malloc user:0x%x, kernel:0x%x\n",
        (int)*user_addr, (int)*kernel_addr);
    return fd;
#else
    if (!user_addr || !kernel_addr) {
        printf("err, null pointer\n");
    }

    *kernel_addr = *user_addr = mmz_memalign(0, 32, size);
    av_log(NULL, AV_LOG_DEBUG, "mmz malloc user:0x%x, kernel:0x%x\n",
        (int)*user_addr, (int)*kernel_addr);
    return (*kernel_addr) ? 0 : -1;
#endif
}

static void ff_mmz_flush_cache(void *addr, int size)
{
#ifdef __linux__
    if (addr && size) {
        struct mmz_blk mmz = {0, 0, size, (uint32_t)addr};
        int fd = open("/dev/mmz", O_RDWR);
        if (fd < 0) {
            return;
        }
        av_log(NULL, AV_LOG_DEBUG, "flush cache addr %p size %d\n", addr, size);
        ioctl(fd, MMZ_FLUSH_CACHE, &mmz);
        close(fd);
    }
#else
    av_log(NULL, AV_LOG_DEBUG, "flush cache addr %p size %d\n", addr, size);
    //vPortCacheFlush(addr, size);
#endif
}
#endif
...
#if ARCH_MIPS
    if (pkt->kernel_addr) {
        ff_mmz_flush_cache((void *)pkt->kernel_addr, pkt->size);
    }
#endif
...
static int jpeg_probe_internal(const AVProbeData *p, int *large_picture)
{
    ...
    for (i = 0; i < p->buf_size - 3; i++) {
        ...
        case SOF6:
        case SOF7:
            av_log(NULL, AV_LOG_DEBUG, "height %d, width %d\n",
                AV_RB16(&b[i + 5]), AV_RB16(&b[i + 7]));
            if (AV_RB16(&b[i + 5]) * AV_RB16(&b[i + 7]) > 1920 *1088) {
                if (large_picture) {
                    *large_picture = 1;
                }
            }
...
{
    return jpeg_probe_internal(p, NULL);
}
