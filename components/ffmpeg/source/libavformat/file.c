...
static int file_open(URLContext *h, const char *filename, int flags)
{
    ...
    if (av_match_ext(filename, "ts") || av_match_ext(filename, "mts")
            || av_match_ext(filename, "m2t") || av_match_ext(filename, "m2ts")
            || av_match_ext(filename, "tp") || av_match_ext(filename, "trp")
	    || av_match_ext(filename, "mpg")) {
        h->max_packet_size = 512 * 1024;
        printf("h->max_packet_size %d\n", h->max_packet_size);
    } else {
        //h->max_packet_size = 512 * 1024; c->blocksize = 512 * 1024;
    }
    //printf("c->blocksize %d\n", c->blocksize);
