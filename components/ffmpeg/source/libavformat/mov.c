...
static int mov_read_stts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->stts_data = av_fast_realloc(sc->stts_data, &alloc_size, entries * sizeof(*sc->stts_data));
    if (!sc->stts_data) {
        return AVERROR(ENOMEM);
    }
...
static void mov_fix_index(MOVContext *mov, AVStream *st)
{
    ...
    if (!msc->elst_data || msc->elst_count <= 0 || nb_old <= 0 || msc->elst_data->duration <= 0) {
        return;
    }
...
static int mov_read_senc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->disable_nonencrypt && encryption_index->nb_encrypted_samples > 0) {
        c->disable_nonencrypt = 0;
    }
    return 0;
}
...
#ifdef USE_HC_HW_DSC
static int dsc_buf_check(struct HCAESCTR* aes_ctr, int size)
{
    if (aes_ctr->buf_size < size) {
        if (aes_ctr->dsc_buf) {
            av_free(aes_ctr->dsc_buf);
        }
        aes_ctr->buf_size = ROUNDUP(size, 32);
        aes_ctr->dsc_buf = memalign(32, aes_ctr->buf_size);
        if (!aes_ctr->dsc_buf) {
            aes_ctr->buf_size = 0;
            return AVERROR(ENOMEM);
        }
...
static void hc_aes_ctr_decrypt(int dsc_fd, uint8_t *dst, const uint8_t *src, int count)
{
    struct dsc_crypt dsc_decrypt = {src, dst, count};
    ioctl(dsc_fd, DSC_DO_DECRYPT, (unsigned long)&dsc_decrypt);
}

static struct HCAESCTR *hc_aes_ctr_decrypt_init(void)
{
    struct HCAESCTR *aes_ctr = NULL;
    aes_ctr = av_mallocz(sizeof(struct HCAESCTR));
    if (!aes_ctr) {
        av_log(NULL, AV_LOG_ERROR, "no memory!\n");
        return NULL;
    }

    aes_ctr->dsc_fd = open("/dev/dsc", O_RDWR);
    if (aes_ctr->dsc_fd < 0) {
        av_log(NULL, AV_LOG_ERROR, "open dsc dev failed!\n");
        av_free(aes_ctr);
        return NULL;
    }
...
static void hc_aes_ctr_decrypt_config(struct HCAESCTR* aes_ctr,
    uint8_t *key, int key_len, uint8_t *iv, int iv_size)
{
    aes_ctr->params.algo_type = DSC_ALGO_AES;
    aes_ctr->params.crypto_mode = DSC_DECRYPT;
    aes_ctr->params.chaining_mode = DSC_MODE_CTR;
    aes_ctr->params.residue_mode = DSC_RESIDUE_AS_ATSC;
    aes_ctr->params.key.buffer = key;
    aes_ctr->params.key.size = key_len;
    aes_ctr->params.iv.buffer = iv;
    aes_ctr->params.iv.size = iv_size;
    ioctl(aes_ctr->dsc_fd, DSC_CONFIG, (unsigned long)&aes_ctr->params);
}

static int cenc_decrypt(MOVContext *c, MOVStreamContext *sc, AVEncryptionInfo *sample, uint8_t *input, int size)
{
    int i;
    struct dsc_crypt dsc_decrypt;

    //printf("cenc_decrypt size %d, subsample_count %d\n", size, sample->subsample_count);
    ...
    if (!sc->cenc.aes_ctr) {
        ...
        sc->cenc.aes_ctr = hc_aes_ctr_decrypt_init();
        if (!sc->cenc.aes_ctr) {
            return AVERROR(ENODEV);
        }
    }

    hc_aes_ctr_decrypt_config(sc->cenc.aes_ctr,
        c->decryption_key, c->decryption_key_len, sample->iv, sample->iv_size);

    if (!sample->subsample_count) {
        if (dsc_buf_check(sc->cenc.aes_ctr, size) < 0) {
            return AVERROR(ENOMEM);
        }
        /* decrypt the whole packet */
        memcpy (sc->cenc.aes_ctr->dsc_buf, input, size);
        hc_aes_ctr_decrypt(sc->cenc.aes_ctr->dsc_fd, sc->cenc.aes_ctr->dsc_buf, sc->cenc.aes_ctr->dsc_buf, size);
        memcpy (input, sc->cenc.aes_ctr->dsc_buf, size);
        return 0;
    } else {
        int protected_total_size = 0, clear_total_size = 0, dsc_buf_offset = 0;
        uint8_t *ori_input = input;

        for (i = 0; i < sample->subsample_count; i++) {
            clear_total_size += sample->subsamples[i].bytes_of_clear_data;
            protected_total_size += sample->subsamples[i].bytes_of_protected_data;
        }

        if (clear_total_size + protected_total_size != size) {
            av_log(c->fc, AV_LOG_ERROR, "total subsample size not equal the packet size\n");
            return AVERROR_INVALIDDATA;
        }

        if (dsc_buf_check(sc->cenc.aes_ctr, protected_total_size) < 0) {
            return AVERROR(ENOMEM);
        }
        ...
        for (i = 0; i < sample->subsample_count; i++) {
            ...
            memcpy(sc->cenc.aes_ctr->dsc_buf + dsc_buf_offset, input,
                sample->subsamples[i].bytes_of_protected_data);
            dsc_buf_offset += sample->subsamples[i].bytes_of_protected_data;
            input += sample->subsamples[i].bytes_of_protected_data;
        }

        /* decrypt the encrypted bytes */
        hc_aes_ctr_decrypt(sc->cenc.aes_ctr->dsc_fd, sc->cenc.aes_ctr->dsc_buf,
            sc->cenc.aes_ctr->dsc_buf, protected_total_size);
        ...
        for (i = 0; i < sample->subsample_count; i++) {
            /* skip the clear bytes */
            input += sample->subsamples[i].bytes_of_clear_data;
            memcpy(input, sc->cenc.aes_ctr->dsc_buf + dsc_buf_offset,
                sample->subsamples[i].bytes_of_protected_data);
            dsc_buf_offset += sample->subsamples[i].bytes_of_protected_data;
            input += sample->subsamples[i].bytes_of_protected_data;
        }
...
#ifdef USE_HC_HW_DSC
        if (sc->cenc.aes_ctr) {
            if (sc->cenc.aes_ctr->dsc_fd >= 0) {
                close(sc->cenc.aes_ctr->dsc_fd);
            }
            if (sc->cenc.aes_ctr->dsc_buf) {
                free(sc->cenc.aes_ctr->dsc_buf);
            }
            free(sc->cenc.aes_ctr);
            sc->cenc.aes_ctr = NULL;
        }
#else
...
static const AVOption mov_options[] = {
    ...
    { "enable_drefs", "Enable external track support.", OFFSET(enable_drefs), AV_OPT_TYPE_BOOL,
        {.i64 = 0}, 0, 1, FLAGS },
    { "disable_nonencrypt", "disable non-encpyt stream", OFFSET(disable_nonencrypt),
