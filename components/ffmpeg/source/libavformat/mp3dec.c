...
#define MP3_TYPEFIND_TRY_HEADERS (100)
...
const uint16_t mp3_bitrate_tab[2][3][15] = {
...
static int mp3decheader(MPADecodeHeader *s, uint32_t header)
{
    ...
    if (sample_rate_index >= FF_ARRAY_ELEMS(mp3_freq_tab)) {
        sample_rate_index = 0;
    }
    ...
    if (s->mode == 3) //MPA_MONO
        s->nb_channels = 1;
    else
        s->nb_channels = 2;

    if (bitrate_index != 0) {
        frame_size = mp3_bitrate_tab[s->lsf][s->layer - 1][bitrate_index];
...
static void mp3_type_find (uint8_t *buf, int in_size,
    unsigned int * found_layer, int *found_prob)
{
    uint8_t *data = NULL;
    uint8_t *data_end = NULL;
    unsigned int size = in_size;
    uint32_t skipped;
    int last_free_offset = -1;
    int last_free_framelen = -1;
    ...
    *found_prob = 0;
    ...
    if (!buf || (size <= 0))
    {
        av_log(NULL, AV_LOG_DEBUG,"invalid arg");
        return ;
    }
    ...
    while (skipped < size) {
        ...
        if (*data == 0xFF) {
            ...
            unsigned int layer = 0, bitrate, samplerate, channels;
            unsigned int found = 0;          /* number of valid headers found */
            uint32_t offset = skipped;

            while (found < MP3_TYPEFIND_TRY_HEADERS) {
                MPADecodeHeader s = {0};
                uint32_t head;
                int length;
                if ((int32_t) (offset - skipped + 4) >= 0 &&
                ...
                head = AV_RB32(head_data);
                if (mp3decheader(&s, head)) {
                    break;
                } else {
                    found++;
                    offset += s.frame_size;
            ...
            if (found >= 3) {
                ...
                if (found == 100) {
                    if (skipped) {
                        probability = AVPROBE_SCORE_EXTENSION;
                    } else {
                        probability = AVPROBE_SCORE_EXTENSION + 1;
                    }
                } else if (found >= 5) {
                    probability = AVPROBE_SCORE_EXTENSION / 2;
                } else {
                    probability = AVPROBE_SCORE_EXTENSION / 4;
                }
...
{
    unsigned int found_layer = 0;
    int score = 0;
    mp3_type_find (p->buf, p->buf_size, &found_layer, &score);
    return score;
}
