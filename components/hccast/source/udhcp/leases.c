...
struct dhcpOfferedAddr *add_lease(u_int8_t *chaddr, u_int32_t yiaddr, unsigned long lease, struct server_config_t *server_config)
{
    ...
    clear_lease(chaddr, yiaddr, server_config);

    oldest = oldest_expired_lease(server_config);
...
u_int32_t find_address(int check_expired, struct server_config_t *server_config)
{
    ...
    for (; addr <= ntohl(server_config->end); addr++)
    {
        ...
        /* lease is not taken */
        ret = htonl(addr);
        if ((!(lease = find_lease_by_yiaddr(ret, server_config)) ||

             /* or it expired and we are checking for expired leases */
             (check_expired  && lease_expired(lease))) &&

            /* and it isn't on the network */
            !check_ip(ret, server_config))
        {
            return ret;
            break;
        }
...
int check_ip(u_int32_t addr, struct server_config_t *server_config)
{
    ...
    if (arpping(addr, server_config->server, server_config->arp, server_config->interface) == 0)
    {
        ...
        add_lease(blank_chaddr, addr, server_config->conflict_time, server_config);
        return 1;
    }
    else return 0;
}
