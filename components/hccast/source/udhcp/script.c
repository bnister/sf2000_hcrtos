...
static char **fill_envp(struct client_config_t *client_config, struct dhcpMessage *packet)
{
    int num_options = 0;
    int i, j;
    char **envp;
...
void run_script(struct client_config_t *client_config, struct dhcpMessage *packet, const char *name)
{
    int pid;
    char **envp;

    if (client_config && client_config->script == NULL)
        return;
    ...
    {
        envp = fill_envp(client_config, packet);
