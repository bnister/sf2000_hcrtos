...
int usbi_io_init(struct libusb_context *ctx)
{
	...
	// printf(" ===> (%s,%u) ctx->event_flags = 0x%x\n", __FUNCTION__, __LINE__, ctx->event_flags);
...
{
	if(!transfer || !transfer->dev_handle || !transfer->dev_handle->dev){
		printf("[Libusb] Error: libusb_submit_transfer -- No such device (it may have been disconnected)\n");
...
static void usbi_event_source_notification(struct libusb_context *ctx)
{
	...
	// usbi_err(ctx, "\t ====> usbi_event_source_notification \n");
