...
#define NDEBUG   // hichip porting for FreeRTOS
...
#if 0 // remove by hichip porting for FreeRTOS
struct list_head {
...
// !note: just for debug
#define usbi_err(ctx, fmt, ...)    log_e(fmt "\n", ##__VA_ARGS__)
#define usbi_warn(ctx, fmt, ...)   log_w(fmt "\n", ##__VA_ARGS__)
#define usbi_info(ctx, fmt, ...)   log_i(fmt "\n", ##__VA_ARGS__)
// #define usbi_dbg(fmt, ...)         log_d(fmt "\n", ##__VA_ARGS__)
