	...
	unsigned long session_id)
{
	...
	for_each_device(ctx, dev) {
		// printf("==> (usbi_get_device_by_session_id) ctx:%p, dev:%p, dev->session_data:0x%lx, session_id:0x%lx\n",
		// 			 ctx, dev, dev->session_data, session_id);
...
int API_EXPORTED libusb_init(libusb_context **context)
{
	...
	if (!usbi_default_context) {
		...
		default_context_refcnt++;
		usbi_dbg("created default context, ctx:%p", usbi_default_context);
	}
...
static void libusb_log_v(struct libusb_context *ctx, enum libusb_log_level level,
	...
	const char *function, const char *format, ...)
{
	...
	va_start(args, format);
	libusb_log_v(ctx, level, function, format, args);
	va_end(args);
}
