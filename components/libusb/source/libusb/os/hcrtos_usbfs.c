...
/* Notify for hotplug */
QueueHandle_t linux_hotplug_msg_queue = NULL;
...
static int libusb_notify(struct notifier_block *self,
                   unsigned long action, void *__dev)
{
	struct usb_device *dev = (struct usb_device *)__dev;
	struct hotplug_msg msg;

    char devpath[64] = { 0 };
    // sprintf(&devpath[0], "/dev/bus/usb/v%04Xp%04X", 
    //         dev->descriptor.idVendor,
    //         dev->descriptor.idProduct);
    // sprintf(&devpath[0], "/dev/bus/usb/%3.3u/%3.3u", 
    //         dev->bus->busnum,
    //         dev->devnum);
	msg.devaddr = dev->devnum;
	msg.busnum = dev->bus->busnum;

    switch (action) {
    case USB_DEVICE_ADD:
		msg.detached = false;
		// printf("\n(libusb_notify) usb device plug in(udev:%p, bus:%u, addr:%u, sys_name:%s)\r\n",
		// 			dev, dev->bus->busnum, dev->devnum, devpath);
		if(!linux_hotplug_msg_queue)
			linux_hotplug_enumerate(dev->bus->busnum, dev->devnum, devpath);
        break;
    case USB_DEVICE_REMOVE:
		msg.detached = true;
		// printf("\n(libusb_notify) usb device plug out(udev:%p, bus:%u, addr:%u, sys_name:%s)\r\n",
		// 			dev, dev->bus->busnum, dev->devnum, devpath);
		if(!linux_hotplug_msg_queue)
			linux_device_disconnected(dev->bus->busnum, dev->devnum);
        break;
	default:
		printf("\n(libusb_notify) receive not support action(%lx)\n", action);
	    return NOTIFY_OK;
    }
	...
	if(pdFAIL == xQueueSend(linux_hotplug_msg_queue, 
				&msg,
				0)){
		usbi_dbg("Cannot send hotplug notify message as message queue is full ");
	}
    return NOTIFY_OK;
}

static struct notifier_block libusb_hotplug_nb = {
    .notifier_call = libusb_notify,
};
...
static int op_init(struct libusb_context *ctx)
{
	...
	if (!max_iso_packet_len) {
		max_iso_packet_len = 8192; // 98304 // 49152
	}
	...
	if (init_count == 0) {
		...
		usb_register_notify(&libusb_hotplug_nb);
	}
	...
	if(!linux_hotplug_msg_queue){
		linux_hotplug_msg_queue = xQueueCreate(HOTPLUG_QUEUE_ITEMS, sizeof(struct hotplug_msg));
		if(!linux_hotplug_msg_queue)
			usbi_err(ctx, "error creating message queue for hotplug");		
	}
...
static void op_exit(struct libusb_context *ctx)
{
    ...
    usb_unregister_notify(&libusb_hotplug_nb);
	...
	uint8_t devaddr, const char *sysfs_dir, int wrapped_fd)
{
	...
	if (sysfs_dir) {
		...
		usbi_dbg("initialize_device : sysfs_dir -- %s", sysfs_dir);
		fd = open(sysfs_dir, O_RDONLY | O_CLOEXEC);
		if(fd < 0){
			printf("--> Error--(%s,%d) -- cannot open %s\n", __FUNCTION__, __LINE__, sysfs_dir);
			return fd;
		}
	...
	if(dev->speed == LIBUSB_SPEED_UNKNOWN)
		dev->speed = usbfs_get_speed(ctx, fd);
	...
	// if (fd != wrapped_fd){
	// 	log_i("==> libusb, close#%d, wrapped_fd:%d, line#%u\n", fd, wrapped_fd, __LINE__);
	// 	close(fd);
	// }
	...
	uint8_t busnum, uint8_t devaddr, const char *sysfs_dir)
{
	...
	// printf("==> busnum %u devaddr %u session_id %lu\n", busnum, devaddr, session_id);
	...
	// printf(" ==> allocating new device for %u/%u (session %lu)\r\n",
	// 	 busnum, devaddr, session_id);  //TODO: need to figure it out why ??

	dev = usbi_alloc_device(ctx, session_id);
	if (!dev){
		printf("==> usbi_alloc_device error : %u/%u\n", busnum, devaddr);		
		return LIBUSB_ERROR_NO_MEM;
	}

	r = initialize_device(dev, busnum, devaddr, sysfs_dir, -1);
	if (r < 0){
		printf("==> initialize_device error : %u/%u\n", busnum, devaddr);	
		goto out;
	}
	r = usbi_sanitize_device(dev);
	if (r < 0){
		printf("==> usbi_sanitize_device error : %u/%u\n", busnum, devaddr);	
		goto out;
	}
...
void linux_device_disconnected(uint8_t busnum, uint8_t devaddr)
{
	...
	for_each_context(ctx) {
		...
		if (dev) {
			...
			usbi_dbg("device not found for session %lx (bus:%u, devadd:%u)",
						 session_id, busnum, devaddr);
		}
	}
	usbi_mutex_static_unlock(&active_contexts_lock);
}
...
static int usbfs_scan_busdir(struct libusb_context *ctx, uint8_t busnum)
{
	...
	// printf("--> libusb: usbfs_scan_busdir: dirpath--%s\n", dirpath);
	dir = opendir(dirpath);
	if (!dir) {
	...
	while ((entry = readdir(dir))) {
		...
		// printf("--> libusb: usbfs_scan_busdir: entry--%s\n", entry->d_name);
		if (entry->d_name[0] == '.')
			continue;
	...
	struct libusb_device_handle *handle, intptr_t sys_dev)
{
	...
	// printf("==> (op_wrap_sys_device) allocating new device for fd %d", fd);
...
const struct usbi_os_backend usbi_backend = {
	...
	// .alloc_streams = op_alloc_streams, // note: HICHIP do not support USB3.x
	// .free_streams = op_free_streams,   // note: HICHIP do not support USB3.x
